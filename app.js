...
const checkRequestsBody = (request, response, next) => {
  ...
  if (dueDate !== undefined) {
    try {
      ...
      const formattedDate = dateFormat(new Date(dueDate), 'yyyy-MM-dd')
...
app.get('/agenda/', checkRequestQueries, async (request, response) => {
  ...
  const selectDueDateQuery = `SELECT id, todo, priority, status, category, due_date AS dueDate FROM todo WHERE due_date = '${date}';`
